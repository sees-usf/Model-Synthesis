...
def SearchForAllSolutionsSampleSat():
    ...
    for i in range(nodes.size()) : 
        origin = nodes.get(i)
        nodeIntVar = model.NewIntVar(origin.getSupport(), origin.getSupport(), 'x' + str(origin.getSymbolIndex()))
        ...
        if edges.size() != 0 :
            for j in range(edges.size()):
                edge = edges.get(j)
                intId = 'x' + str(edge.getId())
                boolId = 'b' + str(edge.getId())
                edgeIntVar = model.NewIntVar(0, edge.getEdgeSupport(), intId)
...
limit = input('Please enter how many solutions you would like to print for each trace (Enter 0 for unlimited): ')
print()
solutionsCanPrint = input('Please specify if you would like each solution to be printed (enter y or n): ')
print()
